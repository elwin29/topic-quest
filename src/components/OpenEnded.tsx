...
const OpenEnded = ({ game }: Props) => {
  ...
  const { mutate: checkAnswer, status: checkStatus } = useMutation({
    mutationFn: async () => {
    ...
    <div className="absolute -translate-x-1/2 -translate-y-1/2 md:w-[80vw] max-w-4xl w-[90vw] top-1/2 left-1/2">
      ...
      <div className="flex flex-col items-center justify-center w-full mt-4">
        ...
        <Button
          variant="outline"
          className="mt-4"
          disabled={checkStatus === "pending" || hasEnded}
          onClick={() => {
            handleNext();
          }}
        >
          {checkStatus === "pending" && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
